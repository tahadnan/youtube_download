...
info_perf(){
    ...
    if [[ $format == "mp3" ]]; then
	    ...
	    yt-dlp -f "bestvideo[height<=?1080]bestaudio[ext=m4a]/best[ext=mp4]/best" "$url"
    else
        error_message
    fi
}
read -r -p "What do you want to download from YouTube([p]playlist or [v]video)
Put in mind that the current supported formats are mp4 and mp3: " uoption
